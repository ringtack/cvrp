...
from model_timer import (
    Timer,
)  # You need to implement a timer module or use an existing one
from vrp_parser import (
    VRPInstance,
)  # Assuming VRPInstance is defined in a separate module
...
def main():
    ...
    print(
        ...
        + file_name
        + '", "Time": '
        + "{:.2f}".format(watch.get_elapsed())
        + ', "Result": '
        + "{:.2f}".format(res.objective())
        + ', "Solution": "'
        + sol
        + '"}'
    )
